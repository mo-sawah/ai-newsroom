            ...
            <div class="ain-tab-panel" data-panel="ai">
                <div class="ain-grid ain-grid-2">
                    ...
                    <div class="ain-card"><h2>Writing Rules</h2>
                        <?php self::textarea( 'writer_prompt', 'Article Writer Prompt', $campaign->ai_config['writer_prompt'], 'Controls the first AI pass: the actual article draft only.' ); ?>
                        <?php self::select_raw( 'production_editor_mode', 'Production Editor', array( 'global' => 'Follow Global Setting', 'enabled' => 'Enabled For This Campaign', 'disabled' => 'Disabled For This Campaign' ), $campaign->ai_config['production_editor_mode'] ?? 'global' ); ?>
                        <?php self::textarea( 'production_prompt', 'Production Editor Prompt', $campaign->ai_config['production_prompt'], 'Only used when the production editor is enabled. It controls Rank Math fields, internal link instructions, image metadata, optional table/chart instructions, and the Fact Check box data. It should not rewrite the article or add external source links inside the article body.' ); ?>
                        <div class="ain-subcard">
                            <h3>Fact Check Box</h3>
                            <p class="ain-help-text">Adds a professional collapsible source-check panel below the article. External URLs are kept out of the article body and shown only inside this box.</p>
                            <?php self::select_raw( 'fact_check_mode', 'Fact Check Box', array( 'global' => 'Follow Global Setting', 'enabled' => 'Enabled For This Campaign', 'disabled' => 'Disabled For This Campaign' ), $campaign->ai_config['fact_check_mode'] ?? 'global' ); ?>
                            <?php self::input( 'fact_check_title', 'Custom Box Title', $campaign->ai_config['fact_check_title'] ?? '', 'text', 'Fact Check & Sources' ); ?>
            ...
            <div class="ain-tab-panel" data-panel="media">
                <div class="ain-grid ain-grid-2">
                    <div class="ain-card"><h2>Media Engine</h2>
                        ...
                        <?php self::checkbox( 'insert_inline_media', 'Allow Smart Production Blocks', $campaign->media_config['insert_inline_media'] ); ?>
                        <?php self::checkbox( 'enable_smart_tables', 'Enable Smart Tables Only When Clearly Useful', $campaign->media_config['enable_smart_tables'] ?? 1 ); ?>
                        <?php self::checkbox( 'enable_smart_charts', 'Enable Smart Charts Only When Clearly Useful', $campaign->media_config['enable_smart_charts'] ?? 1 ); ?>
                    ...
                    <div class="ain-card"><h2>Automation</h2>
                        <p class="ain-muted">Promotion copy generation is no longer handled here. Use your dedicated automation plugin after publishing.</p>
