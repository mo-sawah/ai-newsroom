...
function ain_frontend_assets() {
    if ( is_singular( 'post' ) ) {
        ...
        global $post;
        if ( $post && ( false !== strpos( (string) $post->post_content, 'twitter-tweet' ) || false !== strpos( (string) $post->post_content, 'ain-x-embed' ) || 'x_twitter_story' === get_post_meta( $post->ID, '_ain_source_mode', true ) ) ) {
            wp_enqueue_script( 'ain-twitter-widgets', 'https://platform.twitter.com/widgets.js', array(), null, true );
        }
    }
}
